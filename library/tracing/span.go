...
func StartSpanWithParentKV(parent opentracing.SpanContext, operationName string, tagkeyvalue map[string]interface{}) opentracing.Span {

	options := []opentracing.StartSpanOption{}

	for field, val := range tagkeyvalue {
		options = 	append(options,opentracing.Tag{Key: field, Value: val})
	}
	if parent != nil {
		options = append(options, opentracing.ChildOf(parent))
	}
